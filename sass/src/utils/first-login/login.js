        ...
        success: function (loginResult) {
          wx.getUserInfo({
            ...
            fail: function (userError) {
              let error = new LoginError(constants.ERR_WX_GET_USER_INFO, '获取微信用户信息失败，请检查网络状态');
    ...
    let doLogin = () => getWxLoginResult(function (wxLoginError, wxLoginResult) {
        ...
        wx.request({
            ...
            success: function (result) {
                ...
                if (result && result.statusCode === 200 && result.data.skey) {
                    if (result.data) {
                        ...
                        let errorMessage = '登录失败(' + result.errMsg + ')：' + (result.message || '未知错误');
                ...
                // 没有正确响应会话信息
                    ...
                    let noSessionError = new LoginError(constants.ERR_LOGIN_SESSION_NOT_RECEIVED, JSON.stringify(data));
                    options.fail(noSessionError);
                }
            },
            ...
            fail: function (loginResponseError) {
                let error = new LoginError(constants.ERR_LOGIN_FAILED, '登录失败，可能是网络错误或者服务器发生异常');
                options.fail(error);
            },
        });
    });
 ...
 * @param {Function} options.fail(error) 登录失败后的回调函数，参数 error 错误信息
 */
let requestLogin = function requestLogin(options) {
  ...
  // 构造请求头，包含 code、encryptedData 和 iv
  let code = options.loginParams.code;
  let encryptedData = options.loginParams.encryptedData;
  let iv = options.loginParams.iv;
  let header = {};
